<?php /* HELPDESK $Id: helpdesk.class.php 245 2011-04-09 05:42:28Z caseydk $ */
...
class CHelpDesk extends w2p_Core_BaseObject {
    ...
    public function lookup_contact(CAppUI $AppUI, $contact_id) {
...
// MiraKlim - for compatibility with unmodified files.class.php  
  ...
  function notifymsg($type, $log_msg, $email_list=null) {
    global $AppUI, $w2Pconfig, $HELPDESK_CONFIG;
    if (!isset($email_list)) {
      $email_list = array();   
      ...
      if (($type!=TASK_LOG) || $HELPDESK_CONFIG['task_requestor_notification']) {
		...
		$subject = $HELPDESK_CONFIG['email_subject']."---".$AppUI->_('Item')."#{$this->item_id} ";
		$body .= $HELPDESK_CONFIG{'email_header'} . "\n";
		$body .= $AppUI->_('Item') . "  : " . $this->item_id . "\n";
		$body .= $AppUI->_('Subject') . "  : " . $this->item_title."\n";
		$body .= $AppUI->_('Requestor') . "  : " . $this->item_requestor . "\n";
		$body .= $AppUI->_('Link') . "  : " . $w2Pconfig['base_url']."/index.php?m=helpdesk&a=view&item_id=" . $this->item_id. "\n";
		...
		switch ($type) {
          ...
          $subject .= $AppUI->_('Updated');
				...
				break;
			case TASK_LOG:
			  $subject .= $AppUI->_('Task Log');
    		$body .= "\n" . $AppUI->_('Task Log') . "\n" . $log_msg . "\n";
				break;
			case STATUSTASK_LOG:
			  $subject .= $AppUI->_('Task+Status Log');
    		$body .= "\n" . $AppUI->_('Task+Status Log') . "\n" . $log_msg . "\n";
				break;
			case NEW_WATCHER_LOG:
			  $subject .= $AppUI->_('Watchers Notification');
    		$body = $AppUI->_('You have been added to the watchers list for Help Desk item') . "\n" . $body . "\n" . $log_msg . "\n";
				break;
		}	
  ...
  function log_status_changes() {
    global $ist, $ict, $ics, $ios, $iap, $ipr, $isv, $ist, $field_event_map, $AppUI;
